...
% reading the model
% loading the ecModel (change the 'eciML1515_batch.mat' file with the ecModel of interest)
...
% met to engineer (here the 125th met corresponds to Alanine, one has to find the index of the name of metabolite of interest in the model and replace 125 with it)
met = 1;
i_met = 125;
...
if max_met_bio > 0
    ...
    % letting change for kcat value (corresponds to constraint 9' in the manuscript)
    ...
    % small changes in K
    % constraints 11 and 12 in the manuscript
    M = 10^6;
    beta = 10^(-8);
    ...
    % guarantee that new kcat is positive
    pos = 10^(-6);
    pos_1 = 10^(-12);
    ...
    % v_j = 0 -> v'_ij = 0
    % constraint 10 in the manuscript
    MM = 10^(6);
    ...
    % constraint 13 in the manuscript
    alpha = 10^(1);
    ...
    % initial solution
    % we first find an initial solution that overproduce the metabolite of
    % interest by at most two times and tries to reach this point
    % Then we will use the solution for this as an initial
    % solution for the next one to optimize the changes
    ...
    RESULTS{met,4} = x_eng_temp.x(i_out);

    lower(i_out) = 0.99 * x_eng_temp.x(i_out); % in case of infeasibility one needs to lower the fraction
    lower(find(ecModel.c)) = 0.99 * x_eng_temp.x(find(ecModel.c)); % in case of infeasibility one needs to lower the fraction
    ...
    RESULTS{met,5} = x_eng.x(i_out); % production level in the engineered model
    RESULTS{met,6} = RESULTS{met,5}/max_met_bio; % fold change of the production
    RESULTS{met,7} = x_eng.x(find(ecModel.c)); % biomass level in the engineered model
    ...
    Results_delta = cell(length(inx_changed_delta)+1,6);
    Results_delta{1,1} = 'Enzyme';
    Results_delta{1,2} = 'Reaction';
    Results_delta{1,3} = 'Kcat_In_Model';
    Results_delta{1,4} = 'Proposed_kcat';
    Results_delta{1,5} = 'Difference';
    Results_delta{1,6} = 'Log_fold';
    ...
    for i = 1:length(inx_changed_delta)
        ...
        Results_delta{i+1,1} = enz(6:end);
        Results_delta{i+1,2} = ecModel.rxnNames{inx_Kcats(inx,2)};
        Results_delta{i+1,3} = -1/original_k(inx);
        Results_delta{i+1,4} = Results_delta{i+1,3}/(1 - delta_e(inx)*Results_delta{i+1,3});
        Results_delta{i+1,5} = (delta_e(inx) * Results_delta{i+1,3} * Results_delta{i+1,3})/(1 - delta_e(inx)*Results_delta{i+1,3});
        Results_delta{i+1,6} = log(Results_delta{i,4}/Results_delta{i,3});

        if Results_delta{i+1,5} > 0
            inc_k = inc_k + 1;
        end
        ...
        sum_change_k = sum_change_k + abs((delta_e(inx) * Results_delta{i+1,3} * Results_delta{i+1,3})/(1 - delta_e(inx)*Results_delta{i+1,3}))/Results_delta{i+1,3};
    end

    Results_delta(find(abs(cell2mat(Results_delta(2:end,6))) < 10^(-6)) + 1,:) = [];

    RESULTS{met,8} = inc_k; %number of increased kcats
    RESULTS{met,9} = dec_k; %number of decreased kcats
    RESULTS{met,10} = sum_change_k;

    row_has_empty = any(cellfun(@isempty, Results_delta), 2);
    Results_delta(row_has_empty,:) = [];

    name = strcat(strcat(ecModel.metNames{i_met},'_OKO.txt'));
