...
% reading the model and data for MW
...
T = readtable('ecoli_kcat_preds.csv');
kcats_predicted = table2cell(T);

% mapping the kcats from DLKcat to the ones from the model
for i = 1:size(kcats_predicted,1)
    rxn = kcats_predicted{i,1};
    kcats_predicted{i,1} = ecModel_batch.rxns{strmatch(rxn,ecModel_batch.rxnNames,'exact')};
end
...
for i = 1:size(kcats_predicted,1)
    ...
    if strfind(rxn,'arm_')
        ...
        rxn_indices = setdiff(find(startsWith(ecModel_batch.rxns,rxn_temp)),find(contains(ecModel_batch.rxns,rxn)));
...
for i = 1:size(inx_Kcats,1)
    ...
    if strmatch(rxn_name,kcats_predicted(:,1),'exact')
        if find(contains(kcats_predicted(:,2),prot_name))
           if ~isempty(intersect(strmatch(rxn_name,kcats_predicted(:,1),'exact'),find(contains(kcats_predicted(:,2),prot_name))))
                found = intersect(strmatch(rxn_name,kcats_predicted(:,1),'exact'),find(contains(kcats_predicted(:,2),prot_name)));
                inx_Kcats(i,3) = 1;
...
biomass_level = xlsread('Ecoli_unbounded.xlsx','H1:H41');
inc_level = xlsread('Ecoli_unbounded.xlsx','G1:G41');
